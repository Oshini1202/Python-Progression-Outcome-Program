...
while (user_input == 'y'):                                                      # run the program until user input = 'y'
    ...
    if user_input == 'q':                                                       # run the program until user input = 'q' and show the vertical histogram
        ...
        header = ["Progress\t","Trailer\t","Retriever\t","Excluded"]            # headings of the horizontal histogram
        print(' '.join(header))                                                 # take all the inputs in header item

        list = [Progress, Trailer, Retriever, Excluded]
        maxCount = int(max(list))
        ...
        while (count < maxCount):
            ...
            if (Progress):
                print("*", end="\t\t")                                          # if the condition is true print a *
                Progress -= 1                                                   # dicrease count progress by 1
            else:
                print(" ", end="\t\t")

            if (Trailer):
                print("*", end="\t\t")                                          # if the condition is true print a *
                Trailer -= 1                                                    # dicrease count progress by 1
            else:
                print(" ", end="\t\t")

            if (Retriever):
                print("*", end="\t\t")                                          # if the condition is true print a *
                Retriever -= 1                                                  # dicrease count progress by 1
            else:
                print(" ", end="\t\t")

            if (Excluded):
                print("*", end="\t\t")                                          # if the condition is true print a *
                Excluded -= 1                                                   # dicrease count progress by 1
            else:
                print(" ", end="\t\t")
